...
describe("gbox.js - Sandbox Security (Integration Tests)", () => {
  const testAppBoxPath = path.resolve(
    __dirname,
    "..",
    "fixtures",
    "apps",
  ...
  afterAll(() => {
    fs.rmSync(path.resolve(__dirname, "..", "fixtures"), {
      recursive: true,
      force: true,
    });
  });
  ...
  test('should NOT allow a non-privileged app to require("platform")', () => {
    ...
    fs.writeFileSync(
      scriptPath,
      `module.exports = async function() { require('platform'); };`,
    );
  ...
  test("should throw an error for a protected module that is NOT granted", () => {
    ...
    als.run(mockAlsStore, () => {
      expect(() => {
        ...
        "has not been granted permission to access the 'httpclient' module",
      );
    });
  });
  ...
  test("should DENY access to a privileged module even if listed in grants", () => {
    ...
    const grantedPrivilegedStore = {
      ...mockAlsStore,
      app: { ...mockAlsStore.app, grantedPermissions: ["platform"] },
    };
    const grantedPrivilegedMockConfig = {
      ...mockGBoxConfig,
