...
describe("db.js - Multi-App Integration", () => {
  ...
  beforeAll(async () => {
    ...
    mockApp1 = {
      ...
      logger: appLogger.createAppLogger("app1", "/fake/web/app1/box", {
        level: "error",
      }),
    };
    mockApp2 = {
      // A second app, could point to a different database
      name: "app2",
      config: app1Config, // For this test, it can use the same config
      appBoxPath: "/fake/web/app2/box",
      logger: appLogger.createAppLogger("app2", "/fake/web/app2/box", {
        level: "error",
      }),
    };
    ...
    try {
      ...
      // Probe connectivity
      await als.run(
        { appName: "app1", app: mockApp1, logger: mockApp1.logger },
        async () => {
  ...
  test("should throw an error when a script in app1 tries to access an unknown db name", async () => {
    ...
    await expect(
      ...
      "No DB configured with name 'non_existent_db' for app 'app1'",
    );
  });
});
