...
describe("gbox.js - Sandbox Execution", () => {
  ...
  test('gRequire should throw an error for a non-privileged app requiring "platform" even when granted permission', () => {
    als.run(mockAlsStore, () => {
      ...
      expect(() => {
        runInGBox("/project/web/test_app/box/script.js", {
          ...mockGBoxConfig,
          app: { ...mockGBoxConfig.app, grantedPermissions: ["platform"] },
        });
  ...
  test('gRequire should allow a privileged app to require("platform")', () => {
    ...
    jest.doMock("../../modules/platform.js", () => ({ listApps: () => [] }), {
      virtual: false,
    });
    const privilegedStore = {
      ...mockAlsStore,
      appName: "glide",
    ...
    als.run(privilegedStore, () => {
      ...
      fs.existsSync.mockImplementation((p) => {
        ...
        if (
          String(p).endsWith("platform.js") &&
          String(p).includes(`${path.sep}modules${path.sep}`)
        ) {
          return false;
        }
        return false;
      ...
      if (privilegedError) {
        expect(String(privilegedError.message)).not.toMatch(
  ...
  test("gRequire should resolve to Gingee module for global app modules like image (sandboxed)", () => {
    als.run(mockAlsStore, () => {
      ...
      const gBoxConfig = {
        ...mockGBoxConfig,
        globalModulesPath: path.resolve("./modules"),
      };
  ...
  test("gRequire should reject relative requires that escape the box into a sibling app prefix path", () => {
    als.run(mockAlsStore, () => {
      ...
      const mockScriptPath = path.resolve(
        "/project/web/test_app/box/script.js",
      );
      fs.readFileSync.mockReturnValue(scriptContent);
  ...
  test("allow_dynamic_code false disables Function string codegen", () => {
    als.run(mockAlsStore, () => {
      ...
      expect(() => mod()).toThrow(
        /Security Error|Code generation|disallowed|dynamic code/i,
      );
    });
  });
  ...
  test("resolveAllowDynamicCodeForApp: explicit app wins over server default", () => {
    ...
    expect(
      resolveAllowDynamicCodeForApp({ allow_dynamic_code: true }, {}),
    ).toBe(true);
  ...
  test('should transpile a script containing ESM "import" syntax', () => {
    als.run(mockAlsStore, () => {
      ...
      sucrase.transform.mockReturnValue({
        code: 'const utils = require("utils"); module.exports = () => {};',
      });
      ...
      expect(sucrase.transform).toHaveBeenCalledWith(esmContent, {
