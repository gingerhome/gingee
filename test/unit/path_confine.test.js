...
describe("path_confine helpers", () => {
  ...
  test("resolveConfinedPath rejects .. segments (classic traversal)", () => {
    expect(
      resolveConfinedPath(webRoot, ["..", "other", "secret.txt"]),
    ).toBeNull();
    expect(
      resolveConfinedPath(webRoot, ["css", "..", "..", "settings", "key.pem"]),
    ).toBeNull();
  });
...
describe("resolveScriptTarget confinement", () => {
  ...
  test("routes.json script inside box works when scriptPath precomputed", () => {
    const app = makeApp([
      { path: "/api/ok", script: "api/ok.js", method: "GET" },
    ]);
    expect(app.compiledRoutes[0].scriptPath).toBe(
      path.join(appBox, "api", "ok.js"),
    );
    const result = resolveScriptTarget(
      { method: "GET" },
  ...
  test("routes.json escaping script yields null scriptPath at compile helper", () => {
    ...
    const app = makeApp([
      { path: "/pwn", script: "../../../other/box/pwn.js" },
    ]);
...
describe("handleSpa path confinement", () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(appWeb, "dist", "index.html"),
      "<html>ok</html>",
    );
    fs.writeFileSync(
      path.join(appWeb, "dist", "assets", "app.js"),
      "console.log(1)",
    );
  ...
  function mockRes() {
    ...
    return {
      ...
      writeHead(code) {
        ...
        if (b != null)
          this.body = Buffer.isBuffer(b) ? b.toString("utf8") : String(b);
        this.headersSent = true;
      },
      write(chunk) {
  ...
  function makeRes() {
    const res = {
      ...
      writeHead(code, h) {
        ...
        this.body =
          b == null ? "" : Buffer.isBuffer(b) ? b.toString("utf8") : String(b);
        this.headersSent = true;
      },
      setHeader() {},
      on() {
