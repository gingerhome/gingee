...
describe('request_context/body helpers', () => {
  describe('parseMediaType', () => {
    test('strips charset and other parameters', () => {
      ...
      expect(parseMediaType('multipart/form-data; boundary=----WebKitFormBoundary')).toBe(
        'multipart/form-data'
      );
    });
  ...
  describe('getContentTypeHeader', () => {
    test('reads string or first array element', () => {
      expect(getContentTypeHeader({ headers: { 'content-type': 'application/json' } })).toBe(
        'application/json'
      );
      expect(
        getContentTypeHeader({
  ...
  describe('requestLikelyHasBody', () => {
    test('uses numeric content-length (not string compare)', () => {
      ...
      expect(requestLikelyHasBody({ headers: { 'content-length': '9' } })).toBe(true);
      expect(requestLikelyHasBody({ headers: { 'content-length': '0' } })).toBe(false);
      expect(requestLikelyHasBody({ headers: { 'content-length': '10' } })).toBe(true);
      expect(requestLikelyHasBody({ headers: { 'transfer-encoding': 'chunked' } })).toBe(true);
