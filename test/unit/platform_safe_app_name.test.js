...
describe("platform assertSafeAppName", () => {
  ...
  test("rejects empty, whitespace, and padded names", () => {
    ...
    expect(() => platform.assertSafeAppName("  app")).toThrow(
      /Invalid app name/,
    );
    expect(() => platform.assertSafeAppName("app  ")).toThrow(
      /Invalid app name/,
  ...
  test("rejects path separators and traversal forms", () => {
    expect(() => platform.assertSafeAppName("../evil")).toThrow(
      /Invalid app name/,
    );
    expect(() => platform.assertSafeAppName("..")).toThrow(/Invalid app name/);
    expect(() => platform.assertSafeAppName("foo/bar")).toThrow(
      /Invalid app name/,
    );
    expect(() => platform.assertSafeAppName("foo\\bar")).toThrow(
      /Invalid app name/,
    );
    expect(() => platform.assertSafeAppName("app name")).toThrow(
      /Invalid app name/,
    );
    expect(() => platform.assertSafeAppName("app.name")).toThrow(
      /Invalid app name/,
  ...
  test("with webPath context, safe names resolve under web root", async () => {
    ...
    await als.run(
      { webPath, projectRoot: path.resolve("/project") },
      async () => {
...
describe("platform reserved delete", () => {
  test("isReservedForDelete / assertAppDeletable protect glade", () => {
    ...
    expect(() => platform.assertAppDeletable("glade")).toThrow(
  ...
  test("deleteApp refuses glade without allowReserved", async () => {
    ...
    await als.run(store, async () => {
      await expect(platform.deleteApp("glade")).rejects.toThrow(
        /Cannot delete reserved/,
      );
      await expect(platform.deleteApp("../glade")).rejects.toThrow(
        /Invalid app name/,
  ...
  test("deleteApp refuses names in privileged_apps", async () => {
    ...
    await als.run(store, async () => {
      ...
      await expect(platform.deleteApp("opspanel")).rejects.toThrow(
        /Cannot delete reserved/,
      );
    });
  });
