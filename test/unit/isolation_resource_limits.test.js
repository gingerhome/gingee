...
const { buildDefaultConfig, mergeUserConfig } = require('../../modules/engine/config');
...
describe('isolation resource_limits', () => {
  ...
  test('mergeNodeOptions replaces same flag', () => {
    const merged = mergeNodeOptions('--max-old-space-size=256 --trace-warnings', [
  ...
  test('describeLimits', () => {
    ...
    expect(describeLimits({ max_old_space_mb: 128, priority: 'low' })).toContain(
