...
describe("log_viewer", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(webPath, "glade", "box", "logs"), {
      recursive: true,
    });
    fs.mkdirSync(path.join(webPath, "myapp", "box", "logs"), {
      recursive: true,
    });
  ...
  test("readLogFile tails and parses JSON; default 100", () => {
    ...
    expect(
      r.lines.some((l) => !l.app && l.message && l.message.includes("engine")),
    ).toBe(true);
  });
  ...
  test("engineOnly drops app-forwarded lines", () => {
    ...
    expect(r.lines.some((l) => l.message && l.message.includes("engine"))).toBe(
      true,
    );
  });
  ...
  test("level filter error", () => {
    ...
    expect(
      r.lines.every((l) => (l.level || "").toLowerCase() === "error"),
    ).toBe(true);
  });
  ...
  test("hideLogQueries filters logs-list/logs-read noise by default", () => {
    fs.appendFileSync(
      ...
      [
        JSON.stringify({
          level: "info",
          message:
            "Executing script: E:\\\\repos\\\\gingee\\\\web\\\\glade\\\\box\\\\api\\\\logs-read.js",
    ...
    expect(
      hidden.lines.some((l) =>
        /logs-read\.js|logs-list\.js/.test(l.message || ""),
      ),
    ).toBe(false);
    expect(hidden.lines.some((l) => l.message === "real work happened")).toBe(
      true,
    );
