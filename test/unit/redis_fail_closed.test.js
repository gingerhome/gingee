...
describe("cache_service fail_closed", () => {
  ...
  test("resolveFailClosed defaults true for redis, respects explicit false", () => {
    expect(cacheService.resolveFailClosed({ provider: "redis" }, "redis")).toBe(
      true,
    );
    expect(
      cacheService.resolveFailClosed(
        { provider: "redis", fail_closed: false },
        "redis",
      ),
    ).toBe(false);
    expect(
      cacheService.resolveFailClosed(
        { provider: "redis", fail_closed: true },
        "redis",
      ),
    ).toBe(true);
    expect(
      cacheService.resolveFailClosed({ provider: "memory" }, "memory"),
    ).toBe(false);
  });

  test("redis init failure with fail_closed=true throws and does not use memory", async () => {
    jest
      .spyOn(redisAdapter, "init")
      .mockRejectedValue(new Error("connection refused"));

    const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    await expect(
      cacheService.init(
        { provider: "redis", fail_closed: true, redis: {} },
        logger,
      ),
