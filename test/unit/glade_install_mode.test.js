...
describe('GladeInstallModalMode', () => {
  ...
  test('getInstallLifecycleMode reads data-mode only (not wizard step value)', () => {
    expect(
      getInstallLifecycleMode({ dataset: { mode: 'upgrade' }, value: 'wizard-confirm' })
    ).toBe('upgrade');
    expect(
      getInstallLifecycleMode({ dataset: { mode: 'install' }, value: 'wizard-confirm' })
    ).toBe('install');
