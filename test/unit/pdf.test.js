...
describe("pdf.js - PDF Generation (pdfmake 0.3 API)", () => {
  ...
  beforeEach(() => {
    ...
    PdfPrinter.createPdf.mockImplementation(() => ({
  ...
  describe("init", () => {
    test("should configure access policies, register Roboto fonts, and return success", () => {
      ...
      expect(typeof PdfPrinter.setUrlAccessPolicy.mock.calls[0][0]).toBe(
        "function",
      );
      expect(typeof PdfPrinter.setLocalAccessPolicy.mock.calls[0][0]).toBe(
        "function",
      );
      ...
      expect(fontDescriptors.Roboto).toEqual({
        normal: expect.stringContaining(
          path.join("settings", "fonts", "Roboto", "Roboto-Regular.ttf"),
        ),
        bold: expect.stringContaining(
          path.join("settings", "fonts", "Roboto", "Roboto-Bold.ttf"),
        ),
        italics: expect.stringContaining(
          path.join("settings", "fonts", "Roboto", "Roboto-Italic.ttf"),
        ),
        bolditalics: expect.stringContaining(
          path.join("settings", "fonts", "Roboto", "Roboto-BoldItalic.ttf"),
        ),
      });
    });
  ...
  describe("create", () => {
    ...
    test("should resolve custom fonts via BOX scope and register them before createPdf", async () => {
      ...
      expect(internalUtils.resolveSecurePath).toHaveBeenCalledWith(
        fs.BOX,
    ...
    test("should resolve every style path for each custom font family", async () => {
      ...
      internalUtils.resolveSecurePath.mockImplementation(
        (_scope, p) => `/resolved/${p}`,
      );
    ...
    test("should surface errors from getBuffer", async () => {
      ...
      await expect(pdf.create({ content: "x" })).rejects.toThrow(
        "pdf render failed",
      );
    });
  });
