...
describe("secrets.js", () => {
  ...
  test("resolveValue env: throws when missing and required", () => {
    expect(() => secrets.resolveValue("env:GINGEE_TEST_MISSING")).toThrow(
      /not set/,
    );
  });
  ...
  test("optional secret returns null when required:false", () => {
    secrets.initServer({ required: true, file_roots: [secretsDir] }, tmpRoot, {
      info: jest.fn(),
      warn: jest.fn(),
    });
    const v = secrets.resolveValue({
      $secret: "env:GINGEE_OPTIONAL_MISSING",
      required: false,
    });
    expect(v).toBeNull();
  });
