...
describe('glade login_rate_limit', () => {
  test('resolveConfig defaults and clamps', () => {
    ...
    expect(loginRateLimit.resolveConfig({ LOGIN_MAX_ATTEMPTS: '10' }).maxAttempts).toBe(10);
    expect(loginRateLimit.resolveConfig({ LOGIN_MAX_ATTEMPTS: '0' }).maxAttempts).toBe(1);
    expect(loginRateLimit.resolveConfig({ LOGIN_WINDOW_SEC: '5' }).windowSec).toBe(30); // min 30
  });
  ...
  test('locks after maxAttempts failures on IP', async () => {
    ...
    expect((await loginRateLimit.assertLoginAllowed(cache, id, cfg)).ok).toBe(true);
  ...
  test('username lock is independent of IP', async () => {
    ...
    await loginRateLimit.recordFailure(cache, { ip: '1.1.1.1', username: 'Admin' }, cfg);
  ...
  test('clearFailures allows login again', async () => {
    ...
    await loginRateLimit.recordFailure(cache, id, cfg);
    expect((await loginRateLimit.assertLoginAllowed(cache, id, cfg)).ok).toBe(false);
