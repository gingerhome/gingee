...
describe("internal_utils.js - isPathInside", () => {
  ...
  test("on win32, comparison is case-insensitive", () => {
    ...
    expect(isPathInside(nested, boundary)).toBe(true);
    expect(
      isPathInside("c:\\project\\web\\app10\\box\\file.js", boundary),
    ).toBe(false);
  });
});
...
describe("internal_utils.js - realpath jails (H12)", () => {
  ...
  test("isPathInside rejects directory symlink escape even for non-existent child", () => {
    ...
    expect(isPathInside(path.join(linkDir, "nested", "x.bin"), jail)).toBe(
      false,
    );
  });
  ...
  test("isPathInside still allows normal nested files under a real jail", () => {
    ...
    expect(isPathInside(path.join(jail, "sub", "new-file.dat"), jail)).toBe(
      true,
    );
  });
});
...
describe("internal_utils.js - resolveSecurePath", () => {
  ...
  test("should resolve a path relative to the current script folder", () => {
    als.run(mockStore, () => {
      ...
      expect(resolved).toBe(
        path.resolve("/project/web/app1/box/api/users/get.js"),
      );
    });
  });
  ...
  test("should resolve a relative path to the mirrored WEB location", () => {
    als.run(mockStore, () => {
      ...
      expect(resolved).toBe(
        path.resolve("/project/web/app1/api/assets/style.css"),
      );
    });
  });
  ...
  test("should resolve a path starting with `/` from the BOX root", () => {
    als.run(mockStore, () => {
      ...
      expect(resolved).toBe(
        path.resolve("/project/web/app1/box/data/database.db"),
      );
    });
  });
...
describe("internal_utils.js - loadOptional", () => {
  test("returns the loader result when the package is available", () => {
    expect(
      loadOptional(() => ({ ok: true }), "fake-pkg", "Fake feature"),
    ).toEqual({ ok: true });
  });
