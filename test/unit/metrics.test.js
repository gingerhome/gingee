...
describe("metrics.js", () => {
  ...
  test("inc and setGauge appear in Prometheus text", () => {
    metrics.inc("gingee_http_requests_total", {
      app: "demo",
      kind: "script",
      status_class: "2xx",
    });
  ...
  test("observe builds histogram buckets", () => {
    ...
    const text = metrics.renderPrometheus();
    expect(text).toMatch(
      /gingee_http_request_duration_seconds_count\{app="a",kind="script"\} 2/,
    );
    expect(text).toMatch(
      /gingee_http_request_duration_seconds_sum\{app="a",kind="script"\} /,
    );
  ...
  test("refreshDynamicGauges uses hooks", () => {
    ...
    expect(text).toMatch(
      /gingee_limits_inflight_requests_by_app\{app="demo"\} 2/,
    );
