...
function loadPermissionsForApp(app) {
  ...
  if (fs.existsSync(permissionsFilePath)) {
    try {
      ...
      logger.error(
        `Error parsing settings/permissions.json for app '${app.name}'. Defaulting to no permissions.`,
        { error: e.message },
      );
    }
  }
  ...
  logger.info(
    `Permissions loaded for '${app.name}'. Granted: [${app.grantedPermissions.join(", ") || "none"}]`,
  );
}
...
async function runStartupScripts(app) {
  ...
  logger.info(
    `Running ${scripts.length} startup script(s) for app '${app.name}'...`,
  );
      ...
      await als.run(
        ...
        async () => {
          ...
          if (typeof scriptModule === "function") {
            ...
            throw new Error(
              `Startup script ${scriptPath} in app ${app.name} did not export a function.`,
            );
          }
        },
      );
