...
function classifyIp(ip) {
  ...
  // ff00::/8 multicast
  if (ip6.startsWith("ff"))
    return { kind: "multicast", blockedByDefault: true };
...
function normalizeAddressList(records) {
  ...
  for (const rec of list) {
    if (typeof rec === "string") {
      ...
      const fam =
        rec.family === 4 || rec.family === 6
          ? rec.family
          : ipFamily(rec.address);
  ...
  return function pinnedLookup(hostname, options, callback) {
    ...
    if (typeof callback !== "function") {
      ...
      return Promise.reject(
        new Error("EGRESS_DENIED: pinned lookup requires callback"),
      );
    }
    if (!pinned.length) {
...
async function assertUrlAllowed(urlString, options = {}) {
  ...
  if (net.isIP(host)) {
    ...
    if (!ipCheck.ok) {
      return deny(
        ipCheck.reason,
        `IP not allowed: ${host} (${ipCheck.reason})`,
      );
    }
...
async function resolveAndCheckHost(host, sync) {
  try {
    ...
    if (sync) {
      ...
      records = Array.isArray(addr)
        ? addr
        : [{ address: addr.address || addr, family: addr.family }];
    } else {
...
function deny(reason, message) {
  try {
    ...
    metrics.inc("gingee_egress_denied_total", {
...
function beforeRedirect(options /*, responseDetails */) {
  ...
  if (options.href) {
    ...
    const proto = options.protocol.endsWith(":")
      ? options.protocol
      : `${options.protocol}:`;
  ...
  if (net.isIP(host)) {
    ...
    if (!ipCheck.ok) {
      const err = new Error(
        `EGRESS_DENIED: redirect IP ${host} (${ipCheck.reason})`,
      );
      err.code = "EGRESS_DENIED";
    ...
    options.lookup = createPinnedLookup([
      { address: host, family: ipFamily(host) },
    ]);
  } else if (config.mode === "allowlist") {
    if (!config.allow_hosts.some((p) => hostMatchesPattern(host, p))) {
      const err = new Error(
        `EGRESS_DENIED: redirect host not allowlisted ${host}`,
      );
      err.code = "EGRESS_DENIED";
  ...
  if (
    ...
    try {
      ...
      const err = new Error(
        `EGRESS_DENIED: redirect DNS fail ${host}: ${e.message}`,
      );
      err.code = "EGRESS_DENIED";
      err.reason = "DNS_FAIL";
      throw err;
