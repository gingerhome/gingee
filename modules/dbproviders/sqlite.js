...
class SqliteAdapter {
  ...
  constructor(dbConfig, app, logger) {
    ...
    if (path.isAbsolute(dbFileName)) {
      logger.info(
        `Db init error for - ${app.name} - Absolute paths are not allowed for SQLite databases. Please use a relative path within the app's box folder.`,
      );
      throw new Error(
        `RESTRICTED: Attempt to use absolute SQLite db path - ${dbFileName}`,
      );
    }
  ...
  async query(sql, params = []) {
    try {
      ...
      this.logger.error(
        `SQLite: query error ${error.message} for SQL: ${sql}`,
        { stack: error.stack },
      );
      throw error;
    }
  ...
  async execute(sql, params = []) {
    try {
      ...
      this.logger.error(
        `SQLite: execute error ${error.message} for SQL: ${sql}`,
        { stack: error.stack },
      );
      throw error;
    }
  ...
  async transaction(callback) {
    try {
      ...
      this.logger.error(`SQLite: transaction error ${err.message}`, {
        stack: err.stack,
      });
      throw err;
    }
  ...
  async shutdown() {
    try {
      ...
      this.logger.error(`SQLite: shutdown error ${err.message}`, {
        stack: err.stack,
      });
      throw err;
    }
