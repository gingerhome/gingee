...
class PostgresAdapter {
  ...
  async query(sql, params = []) {
    try {
      ...
      this.logger.error(
        `Postgres: query error ${error.message} for SQL: ${sql}`,
        { stack: error.stack },
      );
      throw error;
    }
  ...
  async execute(sql, params = []) {
    try {
      ...
      this.logger.error(
        `Postgres: execute error ${error.message} for SQL: ${sql}`,
        { stack: error.stack },
      );
      throw error;
    }
    ...
    try {
      ...
      const txClient = {
        ...
        execute: async (sql, params) =>
          (await client.query(sql, params)).rowCount,
      };
      ...
      this.logger.error(`Postgres: transaction error ${err.message}`, {
        stack: err.stack,
      });
      throw err;
    } finally {
