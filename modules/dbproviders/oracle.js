...
class OracleAdapter {
  ...
  async _createPool(dbConfig) {
    try {
      ...
      this.logger.error(
        "Error initializing OracleDB pool. Is the Instant Client configured?",
        err,
      );
      throw err;
    }
  ...
  async query(sqlString, params = []) {
    ...
    try {
      ...
      this.logger.error(
        `OracleDB: query error ${error.message} for SQL: ${sqlString}`,
        { stack: error.stack },
      );
      throw error;
    } finally {
  ...
  async execute(sqlString, params = []) {
    ...
    try {
      ...
      this.logger.error(
        `OracleDB: execute error ${error.message} for SQL: ${sqlString}`,
        { stack: error.stack },
      );
      throw error;
    } finally {
  ...
  async transaction(callback) {
    ...
    try {
      ...
      this.logger.error(`OracleDB: transaction error ${err.message}`, {
        stack: err.stack,
      });
      throw err;
    } finally {
