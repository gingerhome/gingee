...
class MssqlAdapter {
  ...
  async _createPool(dbConfig) {
    try {
      ...
      await pool.connect();
      this.logger.info(
        `Initialized MS SQL Server connection pool for: ${dbConfig.database}`,
      );
      return pool;
    } catch (err) {
  ...
  async query(sqlString, params = []) {
    try {
      ...
      this.logger.error(
        `MSSQL: query error ${error.message} for SQL: ${sqlString}`,
        { stack: error.stack },
      );
      throw error;
    }
  ...
  async execute(sqlString, params = []) {
    try {
      ...
      this.logger.error(
        `MSSQL: execute error ${error.message} for SQL: ${sqlString}`,
        { stack: error.stack },
      );
      throw error;
    }
  ...
  async transaction(callback) {
    ...
    try {
      ...
      this.logger.error(`MSSQL: transaction error ${err.message}`, {
        stack: err.stack,
      });
      throw err;
    }
