module.exports = async function () {
  gingee(async ($g) => {
    ...
    try {
      ...
      const allowed = await loginRateLimit.assertLoginAllowed(cacheService, identity, rateCfg);
      if (!allowed.ok) {
        $g.response.headers['Retry-After'] = String(allowed.retryAfterSec || rateCfg.lockoutSec);
      ...
      if (username !== ADMIN_USERNAME) {
        const fail = await loginRateLimit.recordFailure(cacheService, identity, rateCfg);
        if (fail.locked) {
          $g.response.headers['Retry-After'] = String(fail.retryAfterSec || rateCfg.lockoutSec);
          return $g.response.send(
            {
              code: 429,
      ...
      $g.response.cookies[csrf.CSRF_COOKIE_NAME] = csrf.setCsrfCookie(csrfToken, $g.request);
