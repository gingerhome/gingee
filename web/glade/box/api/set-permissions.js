    ...
    try {
      ...
      if (!appName || !Array.isArray(permissions)) {
        return $g.response.send(
          { error: "A valid `appName` and `permissions` array are required." },
          400,
        );
      }
      ...
      $g.log.error(
        `Failed to set permissions for app '${$g.request.body.appName}'`,
        { error: err.message },
      );
      $g.response.send(
        { error: "An internal error occurred while setting permissions." },
        500,
      );
    }
  });
