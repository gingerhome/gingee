module.exports = async function () {
  gingee(async ($g) => {
    ...
    try {
      ...
      const appName = $g.request.body.appName
        ? $g.request.body.appName[0]
        : null;
      const uploadedFile = $g.request.body.files
        ? $g.request.body.files.package
        : null;

      const permissionsString = $g.request.body.permissions
        ? $g.request.body.permissions[0]
        : "[]";
      let permissions = [];
      try {
      ...
      if (!result) {
        ...
        $g.log.error(`Installation failed for app ${appName}`, {
          error: "No result returned from installApp",
        });
        return $g.response.send(
          {
            code: 500,
      ...
      $g.log.error(`Failed to install app`, {
        error: err.message,
        stack: err.stack,
      });
