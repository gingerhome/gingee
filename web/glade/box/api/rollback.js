    ...
    try {
      if ($g.request.method !== "POST") {
        return $g.response.send(
          { error: "Invalid method. This endpoint requires a POST request." },
          405,
        );
      }
        ...
        return $g.response.send(
          {
            ...
            error: "Bad Request",
            message:
              "A valid `appName` and `permissions` array are required in the request body.",
          },
          400,
        );
      }
      ...
      if (result) {
        ...
        $g.log.error(`Rollback failed for app ${appName}`, {
          error: "No result returned from rollbackApp",
        });
        return $g.response.send(
          {
            code: 500,
      ...
      $g.log.error(`Failed to roll back app '${$g.request.query.app}'`, {
        error: err.message,
      });
      $g.response.send(
        {
