module.exports = async function () {
  gingee(async ($g) => {
    ...
    try {
      ...
      const appVersion =
        ($g.apps[appName] &&
          $g.apps[appName].config &&
          $g.apps[appName].config.version) ||
        "latest";
      ...
      if (packageBuffer) {
        ...
        $g.log.error(`Packaging failed for app ${appName}`, {
          error: "No buffer returned from packageApp",
        });
        return $g.response.send(
          {
            code: 500,
      ...
      $g.log.error(`Failed to package app '${$g.request.query.app}'`, {
        error: err.message,
      });
      $g.response.send(
        {
