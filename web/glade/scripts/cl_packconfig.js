...
const InstallerWizard = (() => {
  ...
  async function analyzeGinFile(file) {
    ...
    if (!appJsonFile || !pmftFile) {
      throw new Error(
        "Invalid package: Missing app.json or pmft.json in the box directory.",
      );
    }
  ...
  function renderWizardTabs(panes, appJson, pmft) {
    ...
    const mandatoryPerms =
      (pmft.permissions && pmft.permissions.mandatory) || [];
    ...
    if (dbConnections.length === 0) {
      dbPane.innerHTML =
        "<p>This application does not require any special configuration.</p>";
  ...
  function buildDbConfigFromForm(originalDbConfig) {
    ...
    originalDbConfig.forEach((dbReq, index) => {
      ...
      const inputs = document.querySelectorAll(
        `#install-modal-body [data-db-index="${index}"]`,
      );
