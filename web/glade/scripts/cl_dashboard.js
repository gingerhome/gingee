document.addEventListener("DOMContentLoaded", () => {
  ...
  const installModal = new bootstrap.Modal(
  ...
  async function fetchAndRenderApps() {
    try {
      const response = await GladeCsrf.fetch("/glade/api/apps", {
        credentials: "include",
      });
  ...
  logoutButton.addEventListener("click", async () => {
    await GladeCsrf.fetch("/glade/logout", {
      method: "POST",
      credentials: "include",
    });
    window.location.href = "/glade/login.html";
  ...
  wizardNextBtn.addEventListener("click", () => {
    const activeTab = document.querySelector(
      "#install-wizard-tabs .nav-link.active",
    );
    ...
    try {
      ...
      const grantedPermissions = Array.from(
        document.querySelectorAll(
          "#permissions-form-glade .form-check-input:checked",
        ),
      ).map((cb) => cb.value);
      const dbConfig = InstallerWizard.buildDbConfigFromForm(
        analyzedAppJson.db,
      );
      ...
      const finalPackageBlob = await InstallerWizard.repackGinFile(
        selectedGinFile,
        finalAppJson,
      );
      ...
      const lifecycle =
        GladeInstallModalMode.getInstallLifecycleMode(installModeInput);
  ...
  document
    ...
    .addEventListener("show.bs.modal", (event) => {
      ...
      const resolved = GladeInstallModalMode.resolveInstallModalMode(
        event.relatedTarget,
      );
  ...
  document
    .getElementById("installModal")
    .addEventListener("hidden.bs.modal", resetInstallModal);
  ...
  async function openRollbackModal(appName) {
    ...
    modalBodyText.innerHTML =
      '<div class="text-center p-3"><div class="spinner-border" role="status"></div><p class="mt-2">Analyzing backup...</p></div>';
    ...
    try {
      ...
      const [analysisRes, currentPermsRes] = await Promise.all([
        GladeCsrf.fetch(`/glade/api/analyze-backup?app=${appName}`, {
          credentials: "include",
        }),
        GladeCsrf.fetch(`/glade/api/get-permissions?app=${appName}`, {
          credentials: "include",
        }),
      ]);
      ...
      if (!analysisRes.ok || !currentPermsRes.ok) {
        throw new Error(
          "Failed to retrieve necessary information from the server.",
        );
      }
      ...
      if (analysis.status !== "success" || currentPerms.status !== "success") {
        throw new Error(
          analysis.error || currentPerms.error || "Server analysis failed.",
        );
      }
      ...
      const toRevoke = [...currentGranted].filter(
        (p) => !backupMandatory.has(p),
      );
      const toGrant = [...backupMandatory].filter(
        (p) => !currentGranted.has(p),
      );
      ...
      if (toGrant.length === 0 && toRevoke.length === 0) {
        diffHtml +=
          '<div class="alert alert-success">No permission changes are required for this rollback.</div>';
      } else {
        if (toGrant.length > 0) {
          diffHtml +=
      ...
      diffHtml +=
        '<p class="mt-3">Do you approve these changes and want to proceed?</p>';

      modalBodyText.innerHTML = diffHtml;

      // Step 4: Wire up the confirm button for the final action
      confirmAction = () =>
        executeRollback(appName, Array.from(backupMandatory));
  ...
  async function openPermissionsModal(appName) {
    ...
    try {
      const response = await GladeCsrf.fetch(
        `/glade/api/get-permissions?app=${appName}`,
        { credentials: "include" },
      );
  ...
  permissionsSaveButton.addEventListener("click", async () => {
    ...
    const checkboxes = document.querySelectorAll(
      "#permissions-form .form-check-input:checked",
    );
  ...
  function setPermissionsButtonLoading(isLoading) {
    ...
    const text = permissionsSaveButton.querySelector(
      "#permissions-save-button-text",
    );
    if (isLoading) {
  ...
  async function reloadApp(appName) {
    ...
    try {
      const response = await GladeCsrf.fetch(
        `/glade/api/reload-app?app=${appName}`,
  ...
  function updateLogsScopeUi() {
    ...
    if (logsEngineOnlyWrap)
      logsEngineOnlyWrap.style.display = isApp ? "none" : "";
  }
  ...
  function renderLogLines(lines) {
    ...
    logsTableBody.innerHTML = lines
      .map((row) => {
        const ts = row.ts
          ? String(row.ts).replace("T", " ").replace(/Z$/, "")
          : "—";
  ...
  async function ensureLogsAppsLoaded() {
    ...
    try {
      const res = await GladeCsrf.fetch("/glade/api/apps", {
        credentials: "include",
      });
      if (res.status === 401) {
        window.location.href = "/glade/login.html";
  ...
  async function loadLogsListAndRead(keepFile) {
    ...
    try {
      ...
      const listRes = await GladeCsrf.fetch(`/glade/api/logs-list?${listQs}`, {
        credentials: "include",
      });
      ...
      if (!logsFile || !logsFile.value) {
        renderLogLines([]);
        if (logsMeta)
          logsMeta.textContent = "No log files found for this scope.";
        return;
      }
      ...
      const readRes = await GladeCsrf.fetch(`/glade/api/logs-read?${readQs}`, {
        credentials: "include",
      });
  ...
  if (logsLevel)
    logsLevel.addEventListener("change", () => loadLogsListAndRead(true));
  if (logsLines)
    logsLines.addEventListener("change", () => loadLogsListAndRead(true));
  if (logsEngineOnly)
    logsEngineOnly.addEventListener("change", () => loadLogsListAndRead(true));
  if (logsHideQueries) {
  ...
  function filterScheduleJobs(jobs) {
    const q =
      (scheduleAppFilter && scheduleAppFilter.value.trim().toLowerCase()) || "";
  ...
  async function loadScheduleAdmin() {
    ...
    showScheduleInfo("");
    try {
      const res = await GladeCsrf.fetch("/glade/api/schedule-list", {
        credentials: "include",
      });
      if (res.status === 401) {
        window.location.href = "/glade/login.html";
  ...
  function filterQueueJobsByApp(jobs) {
    const q =
      (queueAppFilter && queueAppFilter.value.trim().toLowerCase()) || "";
    if (q.length < 3) return jobs;
    return jobs.filter((j) =>
      String(j.appName || "")
        .toLowerCase()
  ...
  async function loadQueueAdmin() {
    ...
    try {
      const [statsRes, liveRes, dlqRes] = await Promise.all([
        ...
        GladeCsrf.fetch("/glade/api/queue-live-list?limit=200", {
          credentials: "include",
        }),
        GladeCsrf.fetch("/glade/api/queue-dlq-list?limit=200", {
          credentials: "include",
        }),
      ]);
      if (
      ...
      if (statsData.status !== "success")
        throw new Error(statsData.error || "Failed to load queue stats");
      if (liveData.status !== "success")
        throw new Error(liveData.error || "Failed to load live jobs");
      if (dlqData.status !== "success")
        throw new Error(dlqData.error || "Failed to load DLQ");
