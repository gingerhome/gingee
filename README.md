...
- **Batteries-Included Backend**
  ...
  - **Background queue:** `require('queue').add` with memory or Redis drivers, retries, DLQ, and box job handlers (`queue` permission). Glade **Queue / DLQ** admin for retry/discard. CRON can target `type: "queue"` for multi-node-safe schedules.
